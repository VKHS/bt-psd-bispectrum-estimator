...
Python models – floating‑point and fixed‑point reference implementations of the BT PSD and bispectrum, plus a structural model that mirrors the RTL.

Core HDL modules – synthesizable Verilog/SystemVerilog blocks: memory‑based FFT, CRU, fractional‑delay filter, window filter, magnitude‑squared and accumulation, bispectrum engine, and ROM generators.

Top‑level integration – configuration package and a configurable top module that can be built as:
...
# Blackman–Tukey PSD / Bispectrum Estimator (FPGA + Python)

This repository contains a Python reference model and HDL implementation of a high‑performance Blackman–Tukey power spectral density (PSD) and bispectral estimator for biomedical signals. The architecture follows the memory‑based FFT and shared‑resource CORDIC approach described in:

> A. Khalili Sadaghiani, B. Forouzandeh,  
...
The goal is to make the estimator reproducible and easy to experiment with in both software and hardware.
